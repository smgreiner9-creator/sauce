...
impl PsolaShifter {
    ...
    /// Process pitch shifting, writing result into `output`.
    /// `output` must be at least `input.len()` long.
    /// Returns true if processing occurred, false if input was copied through.
    pub fn process_into(&mut self, input: &[f32], source_freq: f32, target_freq: f32, output: &mut [f32]) -> bool {
        let len = input.len();
        ...
        self.output_buf[..extended_len].fill(0.0);
        self.window_sum_buf.resize(extended_len, 0.0);
        self.window_sum_buf[..extended_len].fill(0.0);

        let grain_len = 2 * radius;
        let hann_denom = (grain_len.max(2) - 1) as f32;
        ...
        // Copy result into caller's output slice (zero allocation)
        output[..len].copy_from_slice(&self.output_buf[..len]);
        true
    }
